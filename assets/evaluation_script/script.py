...
import rrc_modified as rrc_evaluation_funcs
import importlib
...
def default_evaluation_params():
    ...
    """          
    return {
            'IOU_CONSTRAINT' :  0.33, #0.33,
            'AREA_PRECISION_CONSTRAINT' :0.5,
            'WORD_SPOTTING' :False,
            'MIN_LENGTH_CARE_WORD' :1,
            'GT_SAMPLE_NAME_2_ID':  '([0-9]+).txt', # 'gt_([0-9]+).txt',
            'DET_SAMPLE_NAME_2_ID': '([0-9]+).txt',# 'res_([0-9]+).txt',            
            'LTRB':True, #LTRB:2points(left,top,right,bottom) or 4 points(x1,y1,x2,y2,x3,y3,x4,y4)
            'CRLF':False, # Lines are delimited by Windows CRLF format
            'CONFIDENCES':True, #Detections must include confidence value. MAP and MAR will be calculated,
...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    for k in gt:
        rrc_evaluation_funcs.validate_lines_in_file(k,gt[k],evaluationParams['CRLF'],evaluationParams['LTRB'],True,False)
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    def get_intersection_over_union(pD,pG):
        inter1=get_intersection(pD, pG)
        union1=get_union(pD, pG)
        iou1= inter1/union1
        return iou1
        
        
    def get_intersection(pD,pG):
        are=pD.intersection(pG).area
