...
def validate_lines_in_file(fileName,file_contents,CRLF=False,LTRB=True,withTranscription=True,withConfidence=False,imWidth=0,imHeight=0):
    """
...
def validate_tl_line(line,LTRB=True,withTranscription=True,withConfidence=False,imWidth=0,imHeight=0):
    """
...
def get_tl_line_values(line,LTRB=True,withTranscription=True,withConfidence=False,imWidth=0,imHeight=0):
    """
    Validate the format of the line. If the line is not valid an exception will be raised.
    If maxWidth and maxHeight are specified, all points must be inside the imgage bounds.
    Posible values are:
    ...
    if "," in line :
        vals=line.split(",")
        points=[float(vals[0]),float(vals[1]),float(vals[2]),float(vals[3])]
        transcription=vals[5]
        confidence=vals[4]
    else:
        vals=line.split(" ")
        points=[float(vals[0]),float(vals[1]),float(vals[2]),float(vals[3])]
        transcription=vals[4]
        confidence=0.0
